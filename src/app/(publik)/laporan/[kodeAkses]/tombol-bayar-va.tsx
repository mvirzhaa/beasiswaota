...
export function TombolBayarVA({
  ...
  const [state, formAction, pending] = useActionState(async () => {
    const hasil = await mulaiPembayaranVA(kodeAkses, jadwalBayarId);
  ...
  return (
    <form action={formAction} className="inline-flex items-center gap-1.5">
      <Tombol type="submit" disabled={pending} variant="aksen" ukuran="sm" className="text-xs">
